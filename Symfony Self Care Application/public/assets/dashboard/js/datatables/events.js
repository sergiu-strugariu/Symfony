...
// Class definition
let KTDatatableEvents = function () {
    // Shared variables
    let table;
    let datatable;
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (row.status === window.published) {
                            dataValue = `<a href="${window.singleEventPath}/${row.slug}" target="blank" class="custom-link">${data}</a>`;
                        }
                ...
                },
                {
                    ...
                    className: 'text-center',
                    render: function (data, type, row) {
                        return `<span class="badge badge-${row.eventStatus === 'ended' ? 'danger' : 'primary'} rounded-0">${data}</span>`;
                    }
                },
                {
                    targets: 5,
                    ...
                    className: 'text-center',
                    render: function (data, type, row) {
                        return `<a href="${window.eventImagePath}${data}" target="_blank">
                                    <img src="${window.eventImagePath}${data}" alt="image" class="w-75px">
                                </a>`;
                    }
                },
                {
                    ...
                    render: function (data, type, row) {
                        let template = `
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column rounded-0 menu-state-bg-light-primary fw-bold fs-7 w-100px" data-kt-menu="true">
                                <!--begin::Menu item-->
                                <div class="menu-item">
                                    <a href="/dashboard/event/${row.uuid}/edit?locale=ro" class="menu-link px-3 color-183B51">
                                ...
                                <!--begin::Menu item-->
                                <div class="menu-item">
                                    <a href="#" class="menu-link px-3 color-183B51" data-kt-table-action="moderate" data-action="/dashboard/event/actions/moderate/${row.uuid}" data-name="${row.status}">
                                ...
                                <!--begin::Menu item-->
                                <div class="menu-item">
                                    <a href="#" class="menu-link px-3 color-183B51" data-kt-table-action="delete" data-action="/dashboard/event/actions/remove/${row.uuid}">
    ...
    return {
        init: function () {
            table = document.querySelector('#kt_datatable_events');
