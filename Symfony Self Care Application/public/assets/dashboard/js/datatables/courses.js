...
let KTDatatableJobs = function () {
    ...
    let initDatatable = function () {
        datatable = $(table).DataTable({
            ...
            ajax: {url: window.courseAjaxPath},
            columns: [
                {data: 'id'},
                {data: 'title'},
                {data: 'slug'},
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (row.status === window.published) {
                            dataValue = `<a href="${window.singleCoursePath}/${row.slug}" target="blank" class="custom-link">${data}</a>`;
                        }
                ...
                },
                {
                    ...
                    className: 'text-center',
                    render: function (data, type, row) {
                        return `<a href="${window.trainingImagePath}${data}" target="_blank">
                                    <img src="${window.trainingImagePath}${data}" alt="image" class="w-100px">
                                </a>`;
                    }
                },
                {
                        ...
                        let template = `
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                ...
                                <div class="menu-item px-3">
                                    <a href="/dashboard/course/${row.uuid}/edit?locale=ro" class="menu-link px-3">
                                ...
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
                                    <a href="#" class="menu-link px-3" data-kt-table-action="moderate" data-action="/dashboard/course/actions/moderate/${row.uuid}" data-name="${row.status}">
                                        ${row.status === window.published ? window.translations.dashboard.actions.draft : window.translations.dashboard.actions.publish}
                                    </a>
                                </div>
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                <div class="menu-item px-3 ${window.isAdmin ? '' : 'd-none'}">
                                    <a href="#" class="menu-link px-3" data-kt-table-action="delete" data-action="/dashboard/course/actions/remove/${row.uuid}">
    ...
    return {
        init: function () {
            table = document.querySelector('#kt_datatable_courses');
