...
{
    #[Route('/dashboard/pages', name: 'dashboard_page_index')]
    ...
    public function create(Request $request, EntityManagerInterface $em, FileUploader $fileUploader, LanguageHelper $languageHelper, DefaultHelper $helper): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $uploadFile = $fileUploader->uploadFile(
                $file,
                $form,
                $this->getParameter('app_json_path'),
                'file',
                'config',
                false
            );
            ...
            if ($uploadFile['success']) {
                    ...
                    foreach ($pageTemplate['sections'] as $itemSection) {
                        ...
                        if (isset($itemSection['widgets'])) {
                            ...
                            foreach ($itemSection['widgets'] as $itemWidget) {
                                ...
                                if (isset($itemWidget['classes'])) $widget->setClasses($itemWidget['classes']);
                ...
                $this->addFlash('success', "The page has been successfully created.");
                return $this->redirectToRoute('dashboard_page_index');
            }
        }
    ...
    public function editView(Request $request, EntityManagerInterface $em, LanguageHelper $languageHelper, DefaultHelper $helper, $machineName): Response
    {
        // Init propertyAccess
        ...
        if (null === $page) {
            $this->addFlash('danger', 'This page does not exist');
            return $this->redirectToRoute('dashboard_page_index');
        }
    ...
    public function editSave(Request $request, EntityManagerInterface $em, LanguageHelper $languageHelper, FileUploader $fileUploader, $locale): RedirectResponse
    {
        // Init propertyAccess
        $accessor = PropertyAccess::createPropertyAccessor();
        ...
        if ($request->isMethod('POST') && isset($formData) && isset($files)) {
            // Parse and save file
            ...
            // Set flash message
            $this->addFlash('success', "You've updated the page with success.");
            return $this->redirectToRoute('dashboard_page_index');
        }
    ...
    public function delete(EntityManagerInterface $em, DefaultHelper $helper, $machineName): Response
    {
        // Get path to file
        $jsonParseTemplate = $helper->parsePageJsonFile($machineName);
        ...
        $this->addFlash('success', 'The page has been successfully deleted');
        return $this->redirectToRoute('dashboard_page_index');
    }
}
